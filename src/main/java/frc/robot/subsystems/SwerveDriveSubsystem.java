...
public class SwerveDriveSubsystem extends SubsystemBase
{
  ...
  public double getEncoderCount(boolean useLeft, boolean useBack)
  {
    ...
    {
      if (useBack){
        return leftBackMotor.getSelectedSensorPosition(ENCODER_PORT)+(ENCODER_TICKS/8); //45 cc off
      }
      else{
        return leftFrontMotor.getSelectedSensorPosition(ENCODER_PORT); //45 off
      }
    }
    else
    {
      if (useBack){
        return rightBackMotor.getSelectedSensorPosition(ENCODER_PORT); //180 off
      }
      else{
        return rightFrontMotor.getSelectedSensorPosition(ENCODER_PORT); //45 off
      }
    }
  }
